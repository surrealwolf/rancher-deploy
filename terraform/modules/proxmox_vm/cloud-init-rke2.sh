...
systemctl stop systemd-resolved || log "⚠ Failed to stop systemd-resolved"
systemctl disable systemd-resolved || log "⚠ Failed to disable systemd-resolved"

# Configure /etc/resolv.conf directly
log "Configuring /etc/resolv.conf with DNS servers: $DNS_SERVERS"
cat > /etc/resolv.conf <<EOF
# DNS configuration for RKE2 cluster
# Generated by cloud-init-rke2.sh
...
if [ -n "$CLUSTER_HOSTNAME" ]; then
  DOMAIN=$(echo "$CLUSTER_HOSTNAME" | cut -d. -f2-)
  if [ -n "$DOMAIN" ] && [ "$DOMAIN" != "$CLUSTER_HOSTNAME" ]; then
...
chattr +i /etc/resolv.conf 2>/dev/null || log "⚠ Could not make resolv.conf immutable (chattr not available)"

log "✓ /etc/resolv.conf configured with DNS servers: $DNS_SERVERS"

# Verify DNS configuration
log "Verifying DNS configuration..."
if grep -q "nameserver" /etc/resolv.conf; then
  log "✓ DNS servers configured in /etc/resolv.conf:"
  grep "nameserver" /etc/resolv.conf | sed 's/^/  /'
else
  log "⚠ DNS configuration may not have applied correctly"
fi
